...
(docs/SECURITY_MODEL.md "SSRF" row, ADR-0011, ADR-0014).

Two independent gates: a remote ``https://`` URL (GitHub) goes through
:func:`validate_remote_url`; a local filesystem path goes through
:func:`validate_local_path`. Neither ever executes target-repo code.

Historically lived at ``app/ingestion/url_validator.py``; moved here in Phase 26
so ``core/security`` is the single enforcement point. ``app/ingestion/
url_validator.py`` re-exports these names for backwards compatibility.
"""
