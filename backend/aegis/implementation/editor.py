...
class EditorError(Exception):
    """Base class for anchor/apply failures. Callers (the orchestrator, the
    repair loop) must catch this and record a failed attempt -- never crash
    the pipeline on a bad edit-op (Specification: no fake functionality, but
    also no exceptions leaking past agent boundaries)."""
