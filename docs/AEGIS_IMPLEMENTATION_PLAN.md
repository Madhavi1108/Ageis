...
**Status: COMPLETE — 2026-09-08.** `frontend/` now ships all 14 screens (Repository Dashboard,
Create Task, Task List, Task Execution pipeline, Planning, Code Impact, Implementation/Diff, Test
Execution, Debugging Timeline, Review Findings, Verification Result, PR Summary, Engineering Memory,
Repository Knowledge Graph, System Settings) on real backend data only. Typed client:
`src/types/api-generated.ts` is generated by `openapi-typescript` (`npm run gen:api`,
`--empty-objects-unknown`) from a committed `frontend/openapi/openapi.snapshot.json`, with curated
aliases in `src/types/api.ts`; `apiClient.ts` gained `apiPost`/`apiPatch`, a runtime base-URL
override, and `Authorization` + `X-API-Key` injection from a Settings-stored key (no-op with the
backend's default `auth_enabled=False`). TanStack Query hooks per endpoint group
(`src/hooks/api/*`); state-driven polling (`taskRefetchInterval`: active 2s, `AWAITING_APPROVAL`
8s, terminal → stop) on the task + timeline, with `PipelineView` invalidating the stage key that
matches each new `task.state`. The pipeline component mirrors `orchestrator.STAGE_ORDER` /
`_STATE_RANK`; `stageStatus.ts` (pure, unit-tested) derives per-stage `done|active|failed|pending|
skipped` from the task state + timeline STEP entries (covers resume, the plan-revision loop, the
debug branch taken/skipped, `CANCELLED` mid-stage, `PARTIALLY_SUPPORTED`); each stage panel
lazy-loads its stage GET on expand and shows a "not produced yet" empty state on 404/409 rather
than an error. Diff viewer on `react-diff-view` shows files changed, +/- counts, edit-ops with
rationale grouped by `traceability`, scope compliance, inline review findings, risk/confidence
signal-contribution tables, related generated tests, and the exact raw patch (copy + collapse for
large files). Knowledge graph on Cytoscape (`fcose`) with a Settings node cap and lazy per-hop
expansion (double-click), de-duped merges, and a truncation banner. HITL `DecisionControls`
(Radix dialog, required reason + actor) POST `/tasks/{id}/verification/decision` when the task is
`AWAITING_APPROVAL`, then seed the verification cache from the response and show the recorded
decision read-only. Tailwind CSS with class-based dark/light over Radix primitives; hand-built
accessible components. Tests (gated in the `frontend` CI job): Vitest + React Testing Library +
MSW — unit (`apiClient`, `errorMessages`, `taskRefetchInterval`, `stageStatus`, `diffStats`,
`graphAdapter`), integration (task-flow, HITL approve + 403, error-envelope surfacing,
per-screen render + `axe`), and the acceptance test `everyPanelCallsBackend` asserting every
pipeline stage endpoint receives a request (§30 "no decorative data"). One Playwright smoke spec
(`e2e/smoke.spec.ts`) drives a real `AEGIS_SANDBOX_MODE=fake` backend end to end — kept manual /
non-blocking, not in the CI gate.

**Open items (documented limitations, not gaps):** there is no `GET /repositories` list endpoint,
so the Repository Dashboard keeps a client-side registry of known repository IDs in `localStorage`
(seeded on `POST /repositories`, plus a "track by id" field); Task List (`GET /tasks`) is the
practical entry point. System Settings is client-only — Phase 21 exposes no `/config` / `/policy` /
`/providers` endpoints, so providers, policies and limits are shown read-only as "managed
server-side"; only API URL, API key, actor name, theme, poll interval and graph node cap are
editable. The OpenAPI snapshot is refreshed by hand (`npm run gen:api`); there is no CI check that
the committed snapshot matches a live `/openapi.json`. Real-time is polling only — the optional
SSE/WebSocket timeline is deferred (`taskRefetchInterval` is the seam). Playwright E2E and
visual-snapshot regression are manual / non-blocking so the `frontend` CI job stays hermetic (no
backend service). The knowledge graph is always capped + lazily expanded — it never renders a full
large repository. The GitHub issue path prefills the Create Task form's structured-issue fields
rather than calling `POST /github/.../import` (which persists an `Issue` row, not a task).
`react-cytoscapejs` / `cytoscape` / `cytoscape-fcose` ship no types — a small `src/types/shims.d.ts`
covers them. The CI `tsc` step was `npx tsc -b --noEmit`, which TypeScript 5.5 rejects; it is now
`npm run typecheck` (`tsc -b`) plus `npm run typecheck:test`.
